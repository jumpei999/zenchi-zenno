...
program
  ...
  .action((opts: { dataDir: string; name: string }) => {
    ...
    console.log(`Initialized workspace ${store.workspace.id} at ${resolve(opts.dataDir)}`);
  });

program
  .command("ingest")
  .description("Ingest observations via a connector and extract hypotheses")
  .requiredOption("-c, --connector <id>", "markdown-local | chatgpt-export | github")
  .requiredOption("-p, --path <path>", "export path or directory")
  .option("-d, --data-dir <path>", "workspace directory", defaultDataDir())
  .action(async (opts: { connector: string; path: string; dataDir: string }) => {
    const store = openStore(opts.dataDir);
    const connector = getConnector(opts.connector);
    const correlation = `sync-${Date.now()}`;
    store.appendEvent("SyncStarted", { connector: opts.connector, path: opts.path }, {
...
program
  ...
  .action((opts: { query: string; dataDir: string }) => {
    ...
    if (!decisions.length) {
      console.log("No Decision entities matched. Try a broader query or ingest more sources.");
      return;
    }
    for (const d of decisions) {
      ...
      const mine = evidence.filter((x) => d.evidence_refs.includes(x.evidence.id));
      for (const m of mine) {
        console.log("  evidence:");
        console.log(`    observation: ${m.observation?.source_system}/${m.observation?.source_type}`);
        console.log(`    native_id: ${m.observation?.source_native_id}`);
        console.log(`    title: ${m.observation?.title ?? ""}`);
        const snippet = (m.observation?.text ?? "").slice(0, 200).replace(/\n/g, " ");
...
program
  ...
  .action((opts: { dataDir: string; list?: boolean; accept?: string; reject?: string }) => {
    const store = openStore(opts.dataDir);
    if (opts.accept) {
    ...
    const hyps = store.entities.filter((e) => e.confirmation_state === "hypothesized");
    if (!hyps.length) {
