...
Releases are **automated** by [semantic-release](https://semantic-release.gitbook.io/) on merge to `main` (Conventional Commits → version bump, `CHANGELOG.md`, GitHub Release, npm publish). Maintainers own the pipeline (branch protection, `NPM_TOKEN`, npm scope). Contributors do not publish packages under `@zenchi-zenno` without maintainership.
